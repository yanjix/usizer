...
#include "ispd13/timer_interface.h"
...
void Circuit::calcLoadViol() {
  ...
  for (int k = timingOffsetToSequentialArcs;
       k < timingOffsetToExtraSequentialArcs; k++) {
  ...
  for (int n = 0; n < numNets; n++) {
    ...
    for (int k = k0; k < k1; k++) {
      ...
      netstate.loadViolation =
          max(netstate.loadViolation,
              computeLoadViolationUsingEffectiveCapCellWise(arc, arcstate));
    }  // end method

    timingViolationLoadCellWise += netstate.loadViolation;
  }  // end for

  cerr << "[DEBUG] dowstream/ceff/ceff-max " << loadViol << "/"
       << timingViolationLoad << "/" << timingViolationLoadCellWise << "\n";
...
void Circuit::updateCellLoad(Vcell *cell) {
  ...
  for (int l = l0; l < l1; l++) {
    ...
    if (arc.pin == -1) {
      ...
      tree.setNodeExtraCap(
          p.node, arc.cell->actualInstType->pins[arc.pin].capacitance * 1e-15);
...
*/
    ...
    if (timingViolationLoad > originalLoadViolation) continue;
...
bool Circuit::updateCellTypeLagrangeRelaxationLinearApproximation(
    Vcell *cell, const double alpha) {
  const int n = cell->sinkNetIndex;
...
bool Circuit::updateCellTypeLagrangeRelaxationSensitivitiesLinearApproximation(
...
bool Circuit::updateCellTypeLagrangeRelaxationSensitivitiesDefault(
    Vcell *cell, const double gamma, const double alpha) {
  const int n = cell->sinkNetIndex;
  ...
  for (int i = 0; i < numCandidateCells; i++) {
    ...
    // Do not accept load violations.
    if ((loadViol >
         originalLoadViolation) /*|| (slewViol > originalSlewViolation)*/)
      continue;
...
bool Circuit::updateCellTypeLagrangeRelaxationSensitivitiesTestingChanges(
  ...
  for (int i = 0; i < numCandidateCells; i++) {
    ...
    const double sizingEffectOnLocalNegativeSlack =
        computeSizingEffectOnLocalNegativeSlack(n);
    if ((sizingEffectOnLocalNegativeSlack <
         originalSizingEffectOnLocalNegativeSlack * min(1.3, gamma)))
      continue;
...
*/
  ...
  //	cout << (C1+C2) << "\n";
  //
  //	cout << "Ceff: " << t.computeEffectiveCapacitanceBasedOnJessica(driver,
  // EPSILON, 100) << "\n"; 	exit(8);
...
void Circuit::saveSizes() {
  ...
  if (pfile = fopen(filename.c_str(), "w")) {
    ...
    cout << "Problem saving .sizes file!" << endl;
}
...
void Circuit::readSPEF() {
  ...
  while (valid) {
    ...
    if (!nearlyEqual(spefNet.netLumpedCap, totalCap, 1e-3))
      cout << "[WARNING] Total Cap != Lumped Cap (Net: " << spefNet.netName
           << ") ==> " << totalCap << " != " << spefNet.netLumpedCap << "\n";
  ...
  cout << " Wire capacitances read: " << countWires << " (" << readCnt << ")"
       << endl;
  // copy loads to cells
  set<Vcell *>::iterator it;
  Vcell *tmpCell;
  ...
  /*
      ...
      cout << " Wire capacitances read: " << countWires << " (" << i << ")" <<
     endl;
      //copy loads to cells
      set<Vcell*>::iterator it;
      Vcell* tmpCell;
    ...
    for (int k = 0; k < numCellSizes; k++) {
      ...
      if (cellinfo.timingArcs.size() > 0) {
        const double outRiseDelay =
            lookup(cellinfo.timingArcs[0].riseDelay, capSum, slew);
        const double outFallDelay =
            lookup(cellinfo.timingArcs[0].fallDelay, capSum, slew);
...
void Circuit::stepperFoXLogicalEffort(Vcell *cell) {
  ...
  const double riseSlack =
      -(netstate.arrivalTime[RISE] - netstate.requiredTime[RISE]);
  const double fallSlack =
      -(netstate.arrivalTime[FALL] - netstate.requiredTime[FALL]);
...
void Circuit::stepperSlewTarget(Vcell *cell) {
  ...
  //		}
  //
  //	double lookupDelay( const LibParserTimingInfo &timingInfo, const
  // EdgeType edgeType, const double inputSlew, const double loadCapacitance ) {
  //	const LibParserLUT &lut = edgeType == FALL? timingInfo.fallDelay :
  // timingInfo.riseDelay; 	const double x = loadCapacitance; 	const
  // double y = inputSlew;
  //
  ...
  //	computeArcTiming
  //	// Output Slew
  //	outSlew[RISE] = lookup(timingInfo.riseTransition, xOutputLoad,
  // yInputFallSlew ); 	outSlew[FALL] = lookup(timingInfo.fallTransition,
  // xOutputLoad, yInputRiseSlew );
  //
...
void Circuit::stepperSetGlobalCriticality(Vcell *cell) {
  ...
  cell->globalCriticalityOutputPinFall =
      netstate.requiredTime.getFall() - netstate.arrivalTime.getFall();
  cell->globalCriticalityOutputPinRise =
      netstate.requiredTime.getRise() - netstate.arrivalTime.getRise();
...
void Circuit::stepperSetCellSlewTarget(Vcell *cell) {
  ...
  for (i = 0; i < numCandidateCells; i++) {
    ...
    for (k = 0; k < cellInst->timingArcs.size(); ++k) {
      ...
      outRiseSlew =
          lookup(timingInfo.riseTransition, xOutputLoad, yInputRiseSlew);
      outFallSlew =
          lookup(timingInfo.fallTransition, xOutputLoad, yInputFallSlew);
...
void Circuit::computeCellSizingOptions() {
  ...
  for (int i = 0; i < numFootprints; i++) {
    ...
    if (!footprint.cells.front().dontTouch &&
        !footprint.cells.front().isSequential) {
...
double Circuit::computeLoadViolationUsingEffectiveCap(
    const TimingArc &arc, const TimingArcState &state) const {
  // [WARNING]
...
double Circuit::computeLoadViolationUsingEffectiveCapCellWise(
    const TimingArc &arc, const TimingArcState &state) const {
  // [WARNING]
  // - Assuming that pin[0] is the output.
...
double Circuit::computeLoadViolationUsingDownstreamCap(
    const Vcell *cell) const {
  // [WARNING] Assuming that pin[0] is the output.
  return (cell->actualLoad > cell->actualInstType->pins[0].maxCapacitance)
...
void Circuit::sizingLagrangeRelaxationLinearApproximation(
    const bool resetLambdas) {
  const int numArcs = timingArcs.size();
  const int numNets = timingNets.size();
  const int numCells = depthSortedCells.size();

  if (resetLambdas) resetTimingArcLambdas(1);
  ...
  for (int iteration = 0; iteration < 200; iteration++) {
    ...
    // const int level = (int) (max(0.0,
    // getTimingViolation()/getNumPathsWithNegativeSlack())/WORST_SLACK_BAND);
...
void Circuit::sizingLagrangeRelaxationLinearApproximationTestingChanges(
    const bool resetLambdas) {
  const int numArcs = timingArcs.size();
  const int numNets = timingNets.size();
  const int numCells = depthSortedCells.size();
  ...
  DigestDescriptor digest(*this,
                          "Lagrange Relaxation with Sensitivities - Linear "
                          "Approximation Testing Changes");
  digest.addExtraColumn("Gamma", gamma);
  digest.print();
  ...
  for (int iteration = 0; iteration < 100; iteration++) {
    ...
    for (int k = offsetCombinational; k < numCells; k++) {
      Vcell *cell = depthSortedCells[k];
      updateCellTypeLagrangeRelaxationSensitivitiesTestingChanges(
          cell, gamma /*, alpha*/);
    }  // end for
...
void Circuit::sizingLagrangeRelaxationSensitivitiesLinearApproximation(
    const bool resetLambdas) {
  const int numArcs = timingArcs.size();
  const int numNets = timingNets.size();
  const int numCells = depthSortedCells.size();
  ...
  DigestDescriptor digest(
      *this, "Lagrange Relaxation with Sensitivities - Linear Approximation");
  digest.addExtraColumn("Gamma", gamma);
  digest.print();
  ...
  for (int iteration = 0; iteration < 100; iteration++) {
    ...
    for (int k = offsetCombinational; k < numCells; k++) {
      Vcell *cell = depthSortedCells[k];
      updateCellTypeLagrangeRelaxationSensitivitiesLinearApproximation(
          cell, gamma /*, alpha*/);
    }  // end for
...
void Circuit::sizingLagrangeRelaxationSensitivities(const bool resetLambdas) {
  ...
  if (resetLambdas) resetTimingArcLambdas(1e-2);
...
void Circuit::sizingLagrangeRelaxationSensitivitiesDefault(
    const bool resetLambdas, const int iterations) {
  const int numArcs = timingArcs.size();
  const int numNets = timingNets.size();
  const int numCells = depthSortedCells.size();
  ...
  if (resetLambdas) resetTimingArcLambdas(0.1);

  updateLambdasByFlachReimannAlpha(beta);

  DigestDescriptor digest(*this,
                          "Lagrange Relaxation with Sensitivities - Default");
  // digest.addExtraColumn("Gamma", gamma);
  ...
  for (int iteration = 0; iteration < iterationsLR; iteration++) {
    ...
    for (int k = offsetCombinational; k < numCells; k++) {
      ...
      Vcell *cell = depthSortedCells[k];
      changes +=
          (updateCellTypeLagrangeRelaxation(cell /*, gamma /*, alpha*/) == 1)
              ? 1
              : 0;
    }  // end for
    ...
    lastCost = cost;
    cost = /*getSlackSlack(getWorstSlack()) **/ totalLeakage +
           (getTimingViolation() * pow(10.0 + fabs(getWorstSlack()), 1.25));
    // const double cost = getSlackSlack(getWorstSlack()) * totalLeakage +
    // fabs(getTimingViolation() * getWorstSlack() * getWorstSlack());
    if (cost < bestCost) {
      storeSolution();
      bestCost = cost;
      bestCostIteration = iter;
      if ((iteration > iterationsLR / 1.6) &&
          (getWorstSlack() > -getClkPeriod() * 0.1))
        --iteration;
    }  // end if

    const double costRatio = lastCost / cost;
    if ((getWorstSlack() < -getClkPeriod() * 0.15) ||
        (getTimingViolation() > 2.5e3))
      --iteration;
    if (/*(costRatio > 0.999) &&*/ (costRatio < 1.001)) iteration += 5;
    ...
    if (iter % 25 == 0) callPTNonBlockingNoReport();
    if (double(changes + 1.0) / double(numCells) < 0.0001) ++noChanges;
    if ((digest.getElapsedTime() > runTimeLimit * 0.75) || (noChanges > 3))
      break;

    if (bestCostIteration + 3 < iter) {
      iteration += 3;
      if ((bestCost * 1.1 < cost) ||
          (getTimingViolation() > 10e3)) {  // diverging
        iteration += 5;
      }
    }
    ...
    if (iter == 5) {
      iterationsLR = min(
          (int)ceil((runTimeLimit * 0.75) / (digest.getElapsedTime() / 5.0)),
          250);
    }
...
void Circuit::sizingLagrangeRelaxationSensitivitiesNew(
    const bool resetLambdas) {
  const int numArcs = timingArcs.size();
  const int numNets = timingNets.size();
  const int numCells = depthSortedCells.size();
  ...
  for (int iteration = 0; iteration < 200; iteration++) {
    ...
    const double roundedSlewViol = myRound(timingViolationSlew, 2);
    const double roundedTimingViol = myRound(timingTotalNegativeSlack, 2);
    /*if ( (roundedLoadViol == 0.0) && (roundedTimingViol == 0.0) )*/
    upperBound = min(upperBound, totalLeakage + roundedTimingViol * 10000.0);
    LRS = evaluateLRS();
    // Update lambdas.
    updateLambdasByFlachWeighted();
    // updateLambdasByTennakoon();
...
void Circuit::sizingLagrangeRelaxationSensitivitiesSomeChanges(
    const bool resetLambdas) {
  const int numArcs = timingArcs.size();
  const int numNets = timingNets.size();
  const int numCells = depthSortedCells.size();
...
void Circuit::timingRecoveryPathCounter() {
  ...
  if (nearlyZero(timingTotalNegativeSlack)) return;
  ...
  for (int i = 0; i < numNets; i++) {
    ...
    if (upsize(cell)) {
      ...
      if (timingTotalNegativeSlack < oldTNS) {
        ...
        if (nearlyZero(timingTotalNegativeSlack)) break;
...
void Circuit::timingRecoveryPathCounterTestingChanges() {
  ...
  for (int i = 0; i < numNets; i++) {
    ...
    if (timingTotalNegativeSlack == oldTNS) cont++;
...
void Circuit::timingRecoveryPathCounter(const int limit,
                                        const bool highEffort) {
  updateTiming();
  updateRequiredTime();
  ...
  if (pathMappedCells.empty()) return;

  updateCriticalPathCounter();

  DigestDescriptor digest(
      *this, "Timing Recovery - Path Counter (Limited) " + highEffort);
  digest.print();
  ...
  for (int i = 0; i < numNets; i++) {
    ...
    if (upsize(cell, true)) {
      ...
      if ((timingViolationLoad <= originalLoadViolation) &&
          (timingViolationSlew <= timingViolationSlew) &&
          (timingTotalNegativeSlack < oldTNS)) {
        digest.print();
        ++iter;

        if (nearlyZero(timingTotalNegativeSlack) ||
            (digest.getElapsedTime() > limit))
          break;
...
void Circuit::powerRecovery(const double limit) {
  ...
  do {
    ...
    for (int n = timingNumDummyNets; n < numNets; n++) {
      ...
      if (!cell->dontTouch && !cell->actualInstType->isSequential) {
        ...
        if (increaseVth(cell, true)) {
          ...
          if ((timingViolationLoad <= originalLoadViolation) &&
              (timingViolationSlew <= timingViolationSlew) &&
              (timingTotalNegativeSlack <= currentTNS)) {
            changedCounter++;
            changed = true;
          } else {
            //						cout << "Cell#: "
    ...
    if (digest.getElapsedTime() > limit) break;
...
void Circuit::computeArcTiming(const LibParserTimingInfo &timingInfo,
  ...
  // Delay
  outDelay[RISE] =
      lookup(timingInfo.riseDelay, xFallOutputLoad, yInputFallSlew);
  outDelay[FALL] =
      lookup(timingInfo.fallDelay, xRiseOutputLoad, yInputRiseSlew);

  // Output Slew
  outSlew[RISE] =
      lookup(timingInfo.riseTransition, xFallOutputLoad, yInputFallSlew);
  outSlew[FALL] =
      lookup(timingInfo.fallTransition, xRiseOutputLoad, yInputRiseSlew);
}  // end method
...
void Circuit::computeArcTimingDelayAndSlewSensitivityToOutputLoad(
  ...
  const EdgeArray<double> outputLoad0 =
      arcstate.ceff - sensitivityOffsetOutputLoad;
  const EdgeArray<double> outputLoad1 =
      arcstate.ceff + sensitivityOffsetOutputLoad;
  ...
  if (pfile = fopen(filename.c_str(), "w")) {
    ...
    for (int i = 0; i < numNets; ++i) {
      ...
      if (timingNets[i].driver != NULL)
        fprintf(pfile, "%s %f %f\n", timingNets[i].driver->instName.c_str(),
                timingTrees[i].getLumpedCap(), root.propDownstreamCap);
    }
    fclose(pfile);
    cout << " Saving ceff...done" << endl;
  } else
    cerr << "Problem saving .ceff file!" << endl;
}  // end method
...
void Circuit::buildTimingStructure() {
  ...
  for (int i = 0; i < dummyNets - 1; i++) {
    ...
    netstate.arrivalTime[RISE] -=
        lookup(timingInfo.riseDelay, 0, netstate.slew[RISE]);
    netstate.arrivalTime[FALL] -=
        lookup(timingInfo.fallDelay, 0, netstate.slew[FALL]);
  }  // end for
...
void Circuit::buildTreeStructure() {
  ...
  for (int i = timingOffsetToNetLevel[0]; i < numNets; i++) {
    ...
    if (it == mapNodesToTreeDescriptorIndexes.end()) {
      cout << "[BUG] @ buildTreeStructure - Net " << timingNetName[i]
           << " has no tree associated.\n";
    } else {  // end if
      ...
      for (int k = k0; k < k1; k++) {
        ...
        if (nodeIndex == -1) {
          cout << "[BUG] @ buildTreeStructure - Sink '" << nodeName
               << "' not found in the RC tree driven by net '"
               << timingNetName[i] << "'.\n";
        } else {
  ...
  for (int i = 0; i < numNets; i++) {
    ...
    for (int k = 0; k < numNodes; k++) {
      if (tree.getNodeTag(k) == "sink") {
        ...
        cout << "[BUG] @ buildTreeStructure - Node '" << tree.getNodeName(k)
             << "' is a RC tree endpoint, but it is not a tree sink.\n";
      }  // end else-if
    }    // end for
  }      // end for
...
void Circuit::updateTiming(Vcell *cell) {
  ...
  while (!queue.empty()) {
    ...
    if (!nearlyZero(deltaSlew[FALL]) || !nearlyZero(deltaSlew[RISE]) ||
        !nearlyZero(deltaArrivalTime[FALL]) ||
        !nearlyZero(deltaArrivalTime[RISE])) {
      // Put all sink nets in the queue.
      const int k0 = timingSinkNetPointers[n];
      const int k1 = timingSinkNetPointers[n + 1];
      for (int k = k0; k < k1; k++) {
...
void Circuit::updateTiming_Net(const int i, const int threadId) {
  const EdgeArray<double> EMPTY(-numeric_limits<double>::max(),
                                -numeric_limits<double>::max());
  ...
  for (int k = k0; k < k1; k++) {
    ...
    const LibParserTimingInfo &timingInfo =
        driverArc.cell->actualInstType->timingArcs[driverArc.lut];
    tree.simulate(RCTreeDriver(timingInfo, driverArcState.islew), EPSILON, 100);
    const RCTree::Node &root = tree.getRoot();

    driverArcState.ceff = root.propEffectiveCap * 1e15;
    // driverArcState.ceff =
    // tree.computeEffectiveCapacitanceBasedOnJessica(RCTreeDriver(timingInfo,
    // driverArcState.islew)) * 1e15; driverArcState.ceff =
    // tree.computeEffectiveCapacitanceBasedOnMenezes(RCTreeDriver(timingInfo,
    // driverArcState.islew)) * 1e15;

    driverArcState.oslew = root.propSlew * 1e12;
    driverArcState.delay.set(
        lookup(timingInfo.riseDelay, driverArcState.ceff[RISE],
               driverArcState.islew[FALL]),
        lookup(timingInfo.fallDelay, driverArcState.ceff[FALL],
               driverArcState.islew[RISE]));
    driverArcState.loadViolation =
        computeLoadViolationUsingEffectiveCap(driverArc, driverArcState);

    maxArcLoadViolation =
        max(maxArcLoadViolation, computeLoadViolationUsingEffectiveCapCellWise(
                                     driverArc, driverArcState));
    timingViolationLoad += driverArcState.loadViolation;
    ...
    for (int l = l0; l < l1; l++) {
      ...
      for (int edge = 0, reverseEdge = 1; edge < 2; edge++, reverseEdge--) {
        const double arrivalTime = node.propDelay[edge] * 1e12 +
    ...
    for (int outputEdge = 0; outputEdge < 2; outputEdge++) {
      ...
      // Set worst output arrival time.
      const double arrivalTime = driverArcState.arrivalTime[inputEdge] +
                                 driverArcState.delay[outputEdge];
  ...
  netstate.worstRCDelay.set(-numeric_limits<double>::max(),
                            -numeric_limits<double>::max());
  for (int q = q0; q < q1; q++) {
  ...
  for (int k = k0; k < k1; k++) {
    ...
    for (int edge = 0, reverseEdge = 1; edge < 2; edge++, reverseEdge--) {
      if (netstate.arrivalTime[edge] <
          arcstate.arrivalTime[reverseEdge] + arcstate.delay[edge]) {
...
void Circuit::updateTiming_Net_LinearApproximation(const int i) {
  ...
  for (int k = k0; k < k1; k++) {
    ...
    const TimingArc &driverArc = timingArcs[k];
    TimingArcState &driverArcState = getTimingArcState(k);
    const TimingArcState &driverArcSnap =
        getTimingArcState(k, timingStateStored);
    ...
    driverArcState.oslew.set(
        lookup(timingInfo.riseTransition, driverArcState.ceff[RISE],
               driverArcState.islew[FALL]),
        lookup(timingInfo.fallTransition, driverArcState.ceff[FALL],
               driverArcState.islew[RISE]));
    ...
    const EdgeArray<double> slewChange =
        driverArcState.oslew / driverArcSnap.oslew;

    for (int q = q0; q < q1; q++) {
      TimingArcState &sinkArcState = getTimingArcState(timingSinkArcs[q]);
      const TimingArcState &sinkArcSnap =
          getTimingArcState(timingSinkArcs[q], timingStateStored);

      const EdgeArray<double> degradation =
          sinkArcSnap.islew / driverArcSnap.oslew;

      for (int edge = 0, reverseEdge = 1; edge < 2; edge++, reverseEdge--) {
        const double rcdelay = sinkArcSnap.rcdelay[edge] * loadChange;

        const double arrivalTime = rcdelay +
                                   driverArcState.arrivalTime[reverseEdge] +
                                   driverArcState.delay[edge];

        const double slew =
            sinkArcSnap.islew[edge] * slewChange[edge] * degradation[edge];
  ...
  //		const TimingArc &driverArc = timingArcs[k];
  //		TimingArcState &driverArcState = getTimingArcState(k);
  //		const TimingArcState &driverArcSnap = getTimingArcState(k,
  // timingStateStored);
  //
  //		assert(driverArc.sink == i);
  //
...
void Circuit::updateTiming_WorstArrivalTime() {
  ...
  //
  //			const double slack =
  // myRound(timingTailNetMultiplicities[i]
  //* (T
  //- netstate.arrivalTime[edge]),2);
  //
  //            if (slack < 0) {
  //                timingTotalNegativeSlack -= slack;
  //                timingNumPathsWithNegativeSlack++;
...
void Circuit::updateLambdasByFlach() {
  ...
  for (int i = 0; i < numArcs; i++) {
    ...
    const EdgeArray<double> q = arc.sink == -1
                                    ? EdgeArray<double>(T, T)
                                    : getTimingNetState(arc.sink).requiredTime;
...
void Circuit::updateLambdasByFlachReimann() {
  ...
  for (int i = 0; i < numArcs; i++) {
    ...
    if (arc.sink != -1) {
      ...
      a = arcstate.arrivalTime.getReversed() + arcstate.delay +
          netstate.worstRCDelay;
    } else {
      q.set(T, T);
...
void Circuit::updateRequiredTime() {
  ...
  //	for (int i = numNets - 1; i >= timingNumDummyNets; i--) {
  //		const EdgeArray<double> &requiredTimeAtSink =
  // timingRequiredTime[i];
  //
  //      const int k0 = timingArcPointers[i];
  //		const int k1 = timingArcPointers[i + 1];
  //		for (int k = k0; k < k1; k++) {
  ...
  //            /////////
  //            for (int edge = 0; edge < 2; edge++) {
  //				const EdgeType reverseEdge = ((EdgeType)
  // edge).getReversed();
  //
  //				const double requiredTime =
  // requiredTimeAtSink[edge]
  //- (arcstate.delay[edge]+arcstate.rcdelay[reverseEdge]);
  // if (requiredTimeAtDriver[reverseEdge] > requiredTime)
  //					requiredTimeAtDriver[reverseEdge] =
  // requiredTime;
  //                //Reimann: -m(u->v)
  ...
  for (int i = numNets - 1; i >= timingNumDummyNets; i--) {
    ...
    {  // Sink arcs
      ...
      for (int s = s0; s < s1; s++) {
        ...
        for (int edge = 0; edge < 2; edge++) {
          const double requiredTime =
              arcstate.requiredTime[edge] - arcstate.rcdelay[edge];
    ...
    {  // Driver arcs
      ...
      for (int k = k0; k < k1; k++) {
        const TimingArc &arc = timingArcs[k];
        TimingArcState &arcstate = getTimingArcState(k);
        arcstate.requiredTime =
            (netstate.requiredTime - arcstate.delay).getReversed();
      }  // end for
    }    // end block
  }      // end for
  ...
  for (int i = numNets - 1; i >= timingNumDummyNets; i--) {
    if (getWorstSlack() - getNetSlack(i).getMin() > 1e-3) {
      cerr << "[BUG] @ updateRequiredTime(): Worst Slack = " << getWorstSlack()
           << " but Net Slack (" << timingNetName[i]
           << ", depth=" << timingNets[i].depth
           << ") = " << getNetSlack(i).getMin() << "\n";
      cerr << "\tIndex: " << i << "\n";
  ...
  for (int i = timingNumDummyNets; i < timingNets.size(); i++) {
    ...
    for (int k = k0; k < k1; k++) {
      ...
      maxArrivalTime = max(maxArrivalTime,
                           arcstate.arrivalTime.getReversed() + arcstate.delay);
    }  // end for

    for (int edge = 0; edge < 2; edge++) {
      if (maxArrivalTime[edge] + 1e-3 < netstate.arrivalTime[edge]) {
        cerr << "[BUG] @ Circuit::updateRequiredTime() - Invalid max arrival "
                "time.\n";
  ...
  for (int k = 0; k < timingArcs.size(); k++) {
    ...
    for (int edge = 0; edge < 2; edge++) {
      if (netstate.arrivalTime[edge] - 1e-3 >
          arcstate.arrivalTime[edge] - arcstate.rcdelay[edge]) {
        cerr << "[BUG] @ Circuit::updateRequiredTime() - Invalid max arrival "
                "time.\n";
        cerr << "\tNet: " << netstate.arrivalTime << "\n";
        ...
        cerr << "\tDiff: "
             << (netstate.arrivalTime -
                 (arcstate.arrivalTime - arcstate.rcdelay))
             << "\n";
      }  // end if
    }    // end for
  }      // end for
...
void Circuit::updateTiming_Debug() {
  ...
  while (n >= timingNumDummyNets) {
    ...
    criticalPathDelay +=
        arcstate.delay[edgeType] + arcstate.rcdelay[edgeType.getReversed()];
...
double Circuit::computeSizingEffectOnDriverCellDelay(const int netIndex) {
  ...
  for (int k = k0; k < k1; k++) {
    ...
    effect +=
        arcstate.lambda * (arcstate.delay + arcstate.rcdelay.getReversed());
  }  // end for
...
double Circuit::computeSizingEffectOnLambdaDelaySensitivities(
  ...
  //    for ( int k = k0; k < k1; k++ ) {
  //        const TimingArcState &arcstate =
  //        getTimingArcState(timingLocalArcs[k]); effect += arcstate.lambda *
  //        arcstate.delay;
  //    } // end for
  //
  //
  ...
  //		const EdgeArray<double> deltaSlew = getTimingNetState(n).slew -
  // timingPreviousNetSlew[n];
  //
  //		effect += deltaSlew * timingNetLambdaDelaySensitivity[n];
  //	} // end for
  //
...
void Circuit::updateRequiredTimeLR() {
  ...
  for (int i = numNets - 1; i >= timingNumDummyNets; --i) {
    ...
    for (int k = k0; k < k1; k++) {
      ...
      EdgeArray<double> &requiredTimeAtDriver =
          getTimingNetState(arc.driver).requiredTime;
      EdgeArray<double> slack;
        ...
        lambda[edge] = arcstate.lambda[edge] +
                       pk * (slack[edge] -
                             (netstate.arrivalTime[reverseEdge] -
                              getTimingNetState(i).requiredTime[reverseEdge]));
      ...
      for (int k = k0; k < k1; k++) {
        ...
        for (int edge = 0; edge < 2; edge++) {
          ...
          lambda[edge] =
              ...
              pk * ((netstate.arrivalTime[reverseEdge] -
                     getTimingNetState(i).requiredTime
                         [reverseEdge]) /*/ max(totalLambda[edge],1.0e-12)*/);
...
void Circuit::readTimingLR() {
  ...
  while (true) {
    string name1, name2;
    double riseSlack, fallSlack, riseTransition, fallTransition, riseArrival,
        fallArrival;

    valid =
        tp.read_timing_line(name1, name2, riseSlack, fallSlack, riseTransition,
                            fallTransition, riseArrival, fallArrival);
    ...
    if (name2 != "") {
      ...
      // std::cout << name1 << "/" << name2 << " " << riseSlack << " " <<
      ...
      // name1: port name
      continue;
      // std::cout << name1 << " " << riseSlack << " " << fallSlack << " "  <<
      // riseTransition << " " << fallTransition << " " << riseArrival << " " <<
      // fallArrival << endl ;
    }
  }
...
void Circuit::readTimingFromPTForTimingRecovery() {
  ...
  // trocar apenas uma celula do caminho (a com maior
  // delta_delay/delta_leakage?)
  ...
  while (true) {
    ...
    if (name2 != "") {
      ...
      if (cellIndex == cellNameMap.end()) continue;
      ...
      if (max(riseTransition, fallTransition) > maxTransition) {
        ...
        if (depthSortedCells[cellIndex->second]->dontTouch) cout << endl;

        // cout << " -I- Found pin with slew violation: " << name1 << "/" <<
        // name2 << endl;
        Vcell *driverCell = NULL;
        const int sinkNetIndex =
            depthSortedCells[cellIndex->second]->sinkNetIndex;
        const string netName =
            depthSortedCells[cellIndex->second]->returnNetConnectedToPin(name2);
...
double Circuit::evaluateLRS() {
  ...
  double div = 0.0;
  // Combinational Timing Arcs (body)
  /*for (int i = timingOffsetToCombinationArcs; i <
timingOffsetToExtraSequentialArcs; i++) { const TimingArc &arc = timingArcs[i];
          const TimingArcState &arcstate = getTimingArcState(i);

          const EdgeArray<double> mv =
getTimingNetState(arc.sink).arrivalTime.getReversed() -
timingRequiredTime[arc.sink].getReversed(); const EdgeArray<double> mu_v =
-arcstate.slack;

  int n = arc.sink;
          if (timingSinkNetPointers[n] == timingSinkNetPointers[n + 1]) {
      LRS += (arcstate.lambda * (-arcstate.slack)).aggregate() +
arc.cell->actualInstType->leakagePower; div +=
((-arcstate.slack)*(-arcstate.slack)).aggregate();
  }
  else {
      LRS += (arcstate.lambda * (mu_v - mv)).aggregate() +
arc.cell->actualInstType->leakagePower; div += ((mu_v - mv)*(mu_v -
mv)).aggregate();
  }
...
*/
  ...
  for (int i = timingOffsetToCombinationArcs;
    ...
    const EdgeArray<double> mv =
        getTimingNetState(arc.sink).arrivalTime.getReversed() -
        getTimingNetState(arc.sink).requiredTime.getReversed();
...
void Circuit::callPT() {
  ...
  TimerInterface::Status s = TimerInterface::runTimingAnalysisBlocking(
      sizes, true, timing_pins, true, ceff_pins, rootDir, benchName,
      pollingTime);
...
void Circuit::callPT_TR() {
  ...
  TimerInterface::Status s = TimerInterface::runTimingAnalysisBlockingTR(
      sizes, true, timing_pins, true, ceff_pins, rootDir, benchName,
      pollingTime);
...
void Circuit::callPT_PR() {
  ...
  TimerInterface::Status s = TimerInterface::runTimingAnalysisBlockingPR(
      sizes, true, timing_pins, true, ceff_pins, rootDir, benchName,
      pollingTime);
...
void Circuit::callPTCeffOnly() {
  ...
  const string ceffFilename =
      rootDir + "/" + benchName + "/" + benchName + ".ceff";
  ...
  TimerInterface::Status s = TimerInterface::runTimingAnalysisBlocking(
      sizes, false, timing_pins, true, ceff_pins, rootDir, benchName,
      pollingTime);
...
void Circuit::callPTNegSlackOnly() {
  ...
  TimerInterface::Status s =
      TimerInterface::runTimingAnalysisBlockingNegSlackOnly(
          sizes, true, timing_pins, true, ceff_pins, rootDir, benchName,
          pollingTime);
...
void Circuit::callPTNoReport() {
  ...
  TimerInterface::Status s = TimerInterface::runTimingAnalysisBlockingNoReport(
      sizes, true, timing_pins, true, ceff_pins, rootDir, benchName,
      pollingTime);
...
void Circuit::callPTNonBlocking() {
  ...
  // Run timing analysis
  // cout << " Running nonblocking timing analysis on " << benchName << "..." <<
  // endl;

  TimerInterface::Status s = TimerInterface::startTimingAnalysisNonBlocking(
      sizes, true, timing_pins, true, ceff_pins, rootDir, benchName);
  /*
   cout << "  Timing analysis finished with status: " << s << endl;
   if (TimerInterface::TIMER_FINISHED_SUCCESS != s) {
   cout << " -E-: Something went wrong, exiting. Check pt.log in
   ISPD_CONTEST_ROOT directory" << endl;
   //cout << " Trying again..." << endl;
   //callPT();
   }*/
}
...
void Circuit::callPTNonBlockingNoReport() {
  ...
  TimerInterface::Status s =
      TimerInterface::startTimingAnalysisNonBlockingNoReport(
          sizes, true, timing_pins, true, ceff_pins, rootDir, benchName);
  /*
   cout << "  Timing analysis finished with status: " << s << endl;
   if (TimerInterface::TIMER_FINISHED_SUCCESS != s) {
...
#ifdef REMOTE_PRIMETIME
  try {
    if (!primeTimeConnectionEstablished) return;
...
void Circuit::primeTimeWait(const string &match, ostream *out) {
  ...
  try {
    ...
    while (true) {
      ...
      if (line.find("Error: unknown command 'dirty_trick' (CMD-005)") !=
          string::npos)
        break;
...
void Circuit::primeTimeConnect(const string &serverAddress,
                               const unsigned short serverPort) {
#ifdef REMOTE_PRIMETIME
  try {
    cout << "Trying to connect to PrimeTime at " << serverAddress << ":"
         << serverPort << "... " << flush;
  ...
  while ((!pathMappedCells.empty()) || (!slewViolCells.empty())) {
    ...
    while (!pathMappedCells.empty() && cont < 10) {
      ...
      for (it = ret.first; it != ret.second; ++it) {
        if (depthSortedCells[it->second]->dontTouch) continue;
        ...
        // cout << it->first << "\t" << depthSortedCells[it->second]->instName
        // << endl;

        const bool canDecrease = decreaseVth(depthSortedCells[it->second]);

        const double cellLeakage =
            depthSortedCells[it->second]->getLeakagePower();
        if (canDecrease) {
    ...
    while (0 /*!slewViolCells.empty()*/) {
      ...
      for (it = slewViolCells.begin(); it != slewViolCells.end(); ++it) {
        ...
        if (depthSortedCells[*it]->dontTouch) continue;
...
void Circuit::timingRecoveryPrimeTime(const int iterationLimit) {
  ...
  while ((!pathMappedCells.empty()) || (!slewViolCells.empty())) {
    ...
    while (!pathMappedCells.empty()) {
      ...
      pair<multimap<int, double>::iterator, multimap<int, double>::iterator>
          retSlack;
      pair<multimap<double, int>::iterator, multimap<double, int>::iterator>
          ret;
      ...
      // cout << "\n\nStarting timing recovery with worst slack: " << slack <<
      // endl;
      ...
      // cout << " Path expansion started with size: " << toVisit.size() <<
      // endl;

      while (true) {
        // cout << " Path expansion with size: " << toVisit.size() << endl;
        const int lastVisitSize = toVisit.size();
        for (itToVisit = toVisit.begin(); itToVisit != toVisit.end();
             ++itToVisit) {
          retSlack = slackMappedCells.equal_range(*itToVisit);
          // cout << " cells with more than one pin with negative slack: " <<
          // depthSortedCells[*itToVisit]->instName << endl;
          multimap<int, double>::iterator seed2;
          for (seed2 = retSlack.first; seed2 != retSlack.second; ++seed2) {
            const double slack2 = seed2->second;
            // cout << "\t" << slack2 << endl ;
            const pair<multimap<double, int>::iterator,
                       multimap<double, int>::iterator>
                ret = pathMappedCells.equal_range(slack2);
            int ttt = 0;
            if (ret.first == pathMappedCells.end()) continue;
            for (seed = ret.first; seed != ret.second; ++seed) {
              if (depthSortedCells[seed->second]->dontTouch) continue;
              // cout << "inseriu: " << depthSortedCells[seed->second]->instName
              // << "\t" << depthSortedCells[seed->second]->logicalDepth <<
              // endl;
              toVisit.insert(seed->second);
              ++ttt;
              //   if (ttt > 20) break;
        ...
        if (lastVisitSize == toVisit.size()) break;
      }
      ...
      make_heap(reverseDepthSortedCells.begin(), reverseDepthSortedCells.end(),
                std::greater<int>());
      sort_heap(reverseDepthSortedCells.begin(), reverseDepthSortedCells.end(),
                std::greater<int>());
      /*for (int index = 0; index < numCells; ++index) {
          const int cellIndex = reverseDepthSortedCells[index];
          cout << depthSortedCells[cellIndex]->instName << "\t" <<
      depthSortedCells[cellIndex]->logicalDepth << endl;
      }*/
      for (int index = 0; index < numCells; ++index) {
        const int cellIndex = reverseDepthSortedCells[index];
        // cout << depthSortedCells[cellIndex]->instName << "\t" <<
        // depthSortedCells[cellIndex]->logicalDepth << endl;
        if (depthSortedCells[cellIndex]->dontTouch) continue;
        /*
        ...
        */
        ...
        const double cellLeakage =
            depthSortedCells[cellIndex]->getLeakagePower();
        if (canDecrease) {
          cout << "Trying to solve timing violation in "
               << depthSortedCells[cellIndex]->instName
               << " decreasing its Vth." << endl;
          ++changes;
          // cout << "\n\nChanging Vth for cell: " <<
          // depthSortedCells[cellIndex]->instName << " with depth: " <<
          // depthSortedCells[cellIndex]->logicalDepth << endl;
          break;
    ...
    digest.print();
    // continue;
    for (set<int>::iterator it = slewViolCells.begin();
         it != slewViolCells.end(); ++it) {
      ...
      cout << "Trying to solve slew violation in "
           << depthSortedCells[*it]->instName;
      ...
      if (decreaseVth(depthSortedCells[*it])) {
        ...
        for (int i = 0; i < cell->nextCells.size(); ++i) {
          if (downsize(cell->nextCells[i])) {
            ++changes;
            sized = true;
            cout << " downsizing fanout cell: " << cell->nextCells[i]->instName
                 << endl;
          }
        }
  ...
  while ((!pathMappedCells.empty()) || (!slewViolCells.empty())) {
    ...
    while (!pathMappedCells.empty()) {
      ...
      if (minCell != -1) {
        ...
        changed.insert(minCell);
        ++changes;
        cout << " decreasing Vth - cell: "
             << depthSortedCells[minCell]->instName << endl;
        // break;
      }
      pathMappedCells.erase(ret.first, ret.second);
    }
  ...
  while (true) {
    ...
    while (true) {
      ...
      if (name2 != "") {
        ...
        const int pinIndex =
            depthSortedCells[cellIndex->second]->returnPinIndex(name2);

        const double pinMaxCapacitance =
            cell->actualInstType->pins[pinIndex].maxCapacitance;

        if ((riseCeff > pinMaxCapacitance) || (fallCeff > pinMaxCapacitance)) {
          cout << " Cell: " << cell->instName << " has "
               << max(riseCeff - pinMaxCapacitance,
                      fallCeff - pinMaxCapacitance)
               << " max-cap violation " << EdgeArray<double>(riseCeff, fallCeff)
               << " ";
        ...
        // in a port the only way to solve violation is downsizing its fanout
        // cells timing of a port name1: port name
...
void Circuit::compareTimingEngines() {
  ...
  while (true) {
    ...
    if (name2 != "") {
      ...
      if (cell->actualInstType->isSequential) {
        endpointArrivals.insert(
            make_pair(cell->previousCells[0]->depthIndex,
                      EdgeArray<double>(riseArrival, fallArrival)));
      ...
      for (int k = k0; k < k1; k++) {
        ...
        if (timingArcDriverPinName[k] == name2) {
          ...
          cout << "PT: slack: " << EdgeArray<double>(riseSlack, fallSlack)
               << "\tslew: "
               << EdgeArray<double>(riseTransition, fallTransition)
               << "\tarrival: " << EdgeArray<double>(riseArrival, fallArrival)
               << endl;
          cout << "cell delay: " << arcstate.delay
               << "\tislew: " << arcstate.islew
               << "\tarrival:" << arcstate.arrivalTime
               << "\tCeff: " << arcstate.ceff << endl;
          ...
          worstArrivalRatio1 =
              max(worstArrivalRatio1, max(fallRatio, riseRatio));
          worstArrivalRatio2 =
              min(worstArrivalRatio2, min(fallRatio, riseRatio));
          arrivalCounter += 2;
        ...
        if (name2 == "o") {
          //     cout << name1 << "/" << name2 << ": ";
          // cout << "PT: slack: " << EdgeArray<double>(riseSlack,fallSlack) <<
          // "\tslew: " << EdgeArray<double>(riseTransition,fallTransition) <<
          // "\tarrival: " << EdgeArray<double>(riseArrival,fallArrival) <<
          // endl; cout << "cell delay: " << arcstate.delay << "\toslew: " <<
          // arcstate.oslew << "\tRCdelay: " << arcstate.rcdelay << "\tCeff: "
          // << arcstate.ceff << endl ; cout << "Ceff: " << arcstate.ceff <<
          // endl ; 	worstCeff = max(worstCeff,arcstate.ceff);
        }
      ...
      // endpointArrivals.insert(make_pair(cell->previousCells[0]->depthIndex,
      // EdgeArray<double>(riseArrival,fallArrival)));
      ...
      for (int k = timingOffsetToExtraPrimaryOutputArcs; k < k1; k++) {
        ...
        if (name1 == timingNetName[timingArcs[k].driver]) {
          // std::cout << "\n" << name1 << " " << riseSlack << " " << fallSlack
          // << " "  << riseTransition << " " << fallTransition << " " <<
          // riseArrival << " " << fallArrival << " " << arcstate.arrivalTime <<
          // endl ;
          const double fallRatio = fallArrival / arcstate.arrivalTime[FALL];
          const double riseRatio = riseArrival / arcstate.arrivalTime[RISE];
          arrivalRatio += riseRatio + fallRatio;
  ...
  for (int k = 0; k < timingOffsetToSequentialArcs; k++) {
    TimingArcState &arcstate = getTimingArcState(k);
    // cout << /*timingArcs[k].cell->instName << " " <<*/
    // timingNetName[timingArcs[k].sink] << " Ceff: " << arcstate.ceff << endl ;

  }  // end for

  for (int k = timingOffsetToExtraSequentialArcs;
       k < timingOffsetToExtraPrimaryOutputArcs; k++) {
    TimingArcState &arcstate = getTimingArcState(k);
    TimingArc &arc = timingArcs[k];
    const int driverCellIndex =
        timingNets[timingArcs[k].driver].driver->depthIndex;
    map<int, EdgeArray<double> >::iterator arrivals =
        endpointArrivals.find(driverCellIndex);
    // cout << timingNets[timingArcs[k].driver].driver->instName << " " << " : "
    // << /*arcstate.islew << "\t" << arcstate.delay << "\t" <<*/
    // arcstate.arrivalTime << "\t" /*<< arcstate.rcdelay << "\t" <<
    // arcstate.ceff*/ << " PT arrivals: " << arrivals->second << endl ;
    const double fallRatio =
        arrivals->second[FALL] / arcstate.arrivalTime[FALL];
    const double riseRatio =
        arrivals->second[RISE] / arcstate.arrivalTime[RISE];
    arrivalRatio += riseRatio + fallRatio;
  ...
  cout << "\n\nArrival ratio: " << arrivalRatio / arrivalCounter
       << "\t (PT/Ours)\n";
  cout << "Worst Optmistic Arrival ratio: " << worstArrivalRatio1
       << "\t (PT/Ours)\n";
  cout << "Worst Pessimistic Arrival ratio: " << worstArrivalRatio2
       << "\t (PT/Ours)\n\n";
  ...
  /*
   ...
   // ceff values of a pin
   // name1: cellInstance, name2: pin
   std::cout << name1 << "/" << name2 << " Ceff: " <<
   EdgeArray<double>(riseCeff,fallCeff) << endl ;

   } else {
   // timing of a port
   // name1: port name
   std::cout << name1 << " Ceff: " << EdgeArray<double>(riseCeff,fallCeff) <<
   endl ;
   }
   }
   */
