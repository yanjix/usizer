...
bool SpefParser::read_connections(vector<SpefConnection>& connections) {
  ...
  while (valid &&
         !(tokens.size() == 2 && tokens[0] == "*" && tokens[1] == "CONN")) {
  ...
  if (terminateEarly) return false;
  ...
  while (valid) {
    if (tokens.size() == 4 && tokens[0] == "*" && tokens[1] == "D_NET") {
      ...
      if (tokens[0] == "*" && tokens[1] == "D_NET")
        cout << "[BUG] @ SpefParser::read_net_data: possibly wrong-named net "
                "starting with '"
             << tokens[2] << "'\n";
    }  // end else
...
// Return value indicates if the last read was successful or not.
// If the line read corresponds to a pin, then name1 and name2 will be set to
// the cell instance name and the pin name, respectively. If the line read
// corresponds to a port, then name1 will be set to the port name, and name2
// will be set to "".
bool TimingParser::read_timing_line(string& name1, string& name2,
                                    double& riseSlack, double& fallSlack,
                                    double& riseTransition,
                                    double& fallTransition, double& riseArrival,
                                    double& fallArrival) {
  vector<string> tokens;
  bool valid = read_line_as_tokens(is, tokens);
...
// corresponds to a port, then name1 will be set to the port name, and name2
// will be set to "".
bool CeffParser::read_ceff_line(string& name1, string& name2, double& riseCeff,
                                double& fallCeff) {
  vector<string> tokens;
  bool valid = read_line_as_tokens(is, tokens);
...
// Example function that uses VerilogParser class to parse the given ISPD-13
...
void test_verilog_parser(string filename) {
  ...
  do {
    ...
    if (valid) cout << "Primary input: " << primaryInput << endl;
  ...
  do {
    ...
    if (valid) cout << "Primary output: " << primaryOutput << endl;
...
// Example function that uses TimingParser class to parse the given ISPD-13
...
void test_timing_parser(string filename) {
  ...
  while (true) {
    ...
    if (name2 != "") {
      ...
      std::cout << name1 << "/" << name2 << " " << riseSlack << " " << fallSlack
...
void test_ceff_parser(string filename) {
  ...
  while (true) {
    ...
    if (name2 != "") {
      ...
      // name1: cellInstance, name2: pin
      std::cout << name1 << "/" << name2 << " " << riseCeff << " " << fallCeff
                << endl;
