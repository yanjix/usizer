...
TimerInterface::Status TimerInterface::runTimingAnalysisBlocking(
  ...
  const std::string filename =
      contest_root + "/" + benchmark + "/__TIMER_STARTED_";
  ...
  if (!(timing_request || ceff_request)) {
    std::cout << "-E- runTimingAnalysisBlocking: both timing_request and "
                 "ceff_request are false, there is nothing for the timer to "
                 "do, don't call this function in such cases"
              << std::endl;
    return TIMER_INTERFACEERROR;
  }
  ...
  if (!writePinsForTimer(timing_request, timing_pins, ceff_request, ceff_pins,
                         contest_root, benchmark)) {
    std::cout << "-E- runTimingAnalysisBlocking: problem writing pins for "
                 "timing/ceff request"
              << std::endl;
    return TIMER_INTERFACEERROR;
  }
...
// Write sizes and run timing analysis in blocking mode reporting only negative
// slack paths in .timing file
TimerInterface::Status TimerInterface::runTimingAnalysisBlockingNegSlackOnly(
    const std::vector<std::pair<std::string, std::string> > &sizes,
    const bool timing_request, const std::vector<std::string> &timing_pins,
    const bool ceff_request, const std::vector<std::string> &ceff_pins,
    const std::string &contest_root, const std::string &benchmark,
  ...
  /*if (!writePinsForTimer(timing_request, timing_pins, ceff_request, ceff_pins,
  contest_root, benchmark)) { std::cout << "-E- runTimingAnalysisBlocking:
  problem writing pins for timing/ceff request" << std::endl; return
  TIMER_INTERFACEERROR;
  }*/
  return runTimingAnalysisBlockingNegSlackOnly(contest_root, benchmark,
                                               pollingTime);
}
...
TimerInterface::Status TimerInterface::startTimingAnalysisNonBlocking(
   ...
   */
  if (!(timing_request || ceff_request)) {
    std::cout << "-E- startTimingAnalysisNonBlocking: both timing_request and "
                 "ceff_request are false, there is nothing for the timer to "
                 "do, don't call this function in such cases"
              << std::endl;
    return TIMER_INTERFACEERROR;
  ...
  if (!writePinsForTimer(timing_request, timing_pins, ceff_request, ceff_pins,
                         contest_root, benchmark)) {
    std::cout << "-E- startTimingAnalysisNonBlocking: problem writing pins for "
                 "timing/ceff request"
              << std::endl;
    return TIMER_INTERFACEERROR;
  }
...
// Start timing analysis in non-blocking mode
TimerInterface::Status TimerInterface::startTimingAnalysisNonBlockingNoReport(
    const std::vector<std::pair<std::string, std::string> > &sizes,
    const bool timing_request, const std::vector<std::string> &timing_pins,
    const bool ceff_request, const std::vector<std::string> &ceff_pins,
    const std::string &contest_root, const std::string &benchmark) {
  ...
  /*if (!writePinsForTimer(timing_request, timing_pins, ceff_request, ceff_pins,
  contest_root, benchmark)) { std::cout << "-E- startTimingAnalysisNonBlocking:
  problem writing pins for timing/ceff request" << std::endl; return
  TIMER_INTERFACEERROR;
  }*/
  Status status = getTimerStatus(contest_root, benchmark);
...
// Write sizes and run timing analysis in blocking mode reporting only negative
// slack paths in .timing file
TimerInterface::Status TimerInterface::runTimingAnalysisBlockingNoReport(
    const std::vector<std::pair<std::string, std::string> > &sizes,
    const bool timing_request, const std::vector<std::string> &timing_pins,
    const bool ceff_request, const std::vector<std::string> &ceff_pins,
    const std::string &contest_root, const std::string &benchmark,
  ...
  return runTimingAnalysisBlockingNoReport(contest_root, benchmark,
                                           pollingTime);
}

// Write sizes and run timing analysis in blocking mode reporting only negative
// slack paths in .timing file
TimerInterface::Status TimerInterface::runTimingAnalysisBlockingTR(
    const std::vector<std::pair<std::string, std::string> > &sizes,
    const bool timing_request, const std::vector<std::string> &timing_pins,
    const bool ceff_request, const std::vector<std::string> &ceff_pins,
    const std::string &contest_root, const std::string &benchmark,
...
// Write sizes and run timing analysis in blocking mode reporting only negative
// slack paths in .timing file
TimerInterface::Status TimerInterface::runTimingAnalysisBlockingPR(
    const std::vector<std::pair<std::string, std::string> > &sizes,
    const bool timing_request, const std::vector<std::string> &timing_pins,
    const bool ceff_request, const std::vector<std::string> &ceff_pins,
    const std::string &contest_root, const std::string &benchmark,
...
std::string TimerInterface::getTimerStatusString(
  ...
  for (unsigned i = 0; i < files.size(); ++i) {
    if ("__SIZERCMD_TIMERERROR_" == files[i] ||
        ...
        "__TCMD_RUNTIMER_PR_" == files[i] || "__TCMD_RUNTIMER_" == files[i]) {
      if (cmd != "") {
        std::cout << "-Error- getTimerStatusString: multiple status found"
                  << std::endl;
...
bool TimerInterface::removeFile(const std::string &dir,
  ...
  if (file != "__SIZERCMD_TIMERERROR_" && file != "__SIZERCMD_TIMERDONE_") {
    std::cout << "-E- removeFile: You can't use this to remove any files other "
                 "than __SIZERCMD_TIMERERROR_ and __SIZERCMD_TIMERDONE_"
              << std::endl;
    assert(false);
  }
  std::string filename = dir + "/" + file;
  if (doesFileExist(filename) && remove(filename.c_str())) {
    std::cout << "-E- removeFile: could not remove '" << filename << "'"
              << std::endl;
    return false;
  }
  return true;
...
bool TimerInterface::writeSizesForTimer(
  ...
  if (!ofile) {
    std::cout << "-E- writeSizesForTimer: could not open file '" << filename
              << "' for output" << std::endl;
    return false;
  }
...
bool TimerInterface::writePinsForTimer(
    const bool timing_request, const std::vector<std::string> &timing_pins,
    const bool ceff_request, const std::vector<std::string> &ceff_pins,
    const std::string &contest_root, const std::string &benchmark) {
  if (!(timing_request || ceff_request)) {
    std::cout << "-E- writePinsForTimer: both timing_request and ceff_request "
                 "are false"
              << std::endl;
    return false;
  }
  ...
  // Timing
  const std::string timing_pins_filename =
      contest_root + "/" + benchmark + "/" + benchmark + ".timing_pins";
  if (timing_request) {
    std::ofstream ofile(timing_pins_filename.c_str());
    if (!ofile) {
      std::cout << "-E- writePinsForTimer: could not open file '"
                << timing_pins_filename << "' for output" << std::endl;
      return false;
    }
    ...
    if (doesFileExist(timing_pins_filename) &&
        remove(timing_pins_filename.c_str())) {
      std::cout << "-E- writePinsForTimer: could not remove '"
                << timing_pins_filename << "'" << std::endl;
      return false;
    }
  }

  // Ceff
  const std::string ceff_pins_filename =
      contest_root + "/" + benchmark + "/" + benchmark + ".ceff_pins";
  if (ceff_request) {
    std::ofstream ofile(ceff_pins_filename.c_str());
    if (!ofile) {
      std::cout << "-E- writePinsForTimer: could not open file '"
                << ceff_pins_filename << "' for output" << std::endl;
      return false;
    }
    ...
    if (doesFileExist(ceff_pins_filename) &&
        remove(ceff_pins_filename.c_str())) {
      std::cout << "-E- writePinsForTimer: could not remove '"
                << ceff_pins_filename << "'" << std::endl;
      return false;
    }
  }
...
bool TimerInterface::startTimingAnalysis(const std::string &contest_root,
  ...
  std::ofstream ofile(filename.c_str());
  if (!ofile) {
    std::cout << "-E- startTimingAnalysis: problem instructing timer to run "
                 "timing, could not write out '"
              << filename << "'" << std::endl;
    assert(false);
  }
  ofile.close();
...
// Start timing analysis (does not wait for it to finish)
bool TimerInterface::startTimingAnalysisNegSlackOnly(
    const std::string &contest_root, const std::string &benchmark) {
  const std::string filename =
      contest_root + "/" + benchmark + "/__TCMD_RUNTIMER_NEG_SLACK_ONLY_";
  if (doesFileExist(filename)) {
    return false;
  }
...
// Start timing analysis (does not wait for it to finish)
bool TimerInterface::startTimingAnalysisNoReport(
    const std::string &contest_root, const std::string &benchmark) {
  const std::string filename =
      contest_root + "/" + benchmark + "/__TCMD_RUNTIMER_NO_REPORT_";
  if (doesFileExist(filename)) {
    return false;
  }
...
// Start timing analysis (does not wait for it to finish)
bool TimerInterface::startTimingAnalysisTR(const std::string &contest_root,
                                           const std::string &benchmark) {
  const std::string filename =
      contest_root + "/" + benchmark + "/__TCMD_RUNTIMER_TR_";
  if (doesFileExist(filename)) {
    return false;
  }
...
// Start timing analysis (does not wait for it to finish)
bool TimerInterface::startTimingAnalysisPR(const std::string &contest_root,
                                           const std::string &benchmark) {
  const std::string filename =
      contest_root + "/" + benchmark + "/__TCMD_RUNTIMER_PR_";
  if (doesFileExist(filename)) {
    return false;
  }
