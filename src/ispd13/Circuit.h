...
class Circuit {
  class RCTreeDriver {
   ...
   public:
    RCTreeDriver(const LibParserTimingInfo &timingInfo,
                 const EdgeArray<double> &inputSlew)
        : clsTimingInfo(timingInfo),
          clsInputSlew(inputSlew) {}  // end constructor

    EdgeArray<double> computeSlew(const EdgeArray<double> &ceff) const {
      return EdgeArray<double>(lookup(clsTimingInfo.riseTransition,
                                      ceff[RISE] * 1e15, clsInputSlew[FALL]) *
                                   1e-12,
                               lookup(clsTimingInfo.fallTransition,
                                      ceff[FALL] * 1e15, clsInputSlew[RISE]) *
                                   1e-12);
    }  // end method

    EdgeArray<double> computeDelay(const EdgeArray<double> &ceff) const {
      return EdgeArray<double>(lookup(clsTimingInfo.riseDelay,
                                      ceff[RISE] * 1e15, clsInputSlew[FALL]) *
                                   1e-12,
                               lookup(clsTimingInfo.fallDelay,
                                      ceff[FALL] * 1e15, clsInputSlew[RISE]) *
                                   1e-12);
    }  // end method
  ...
  class DigestDescriptor {
   private:
    ...
    struct Column {
      ...
      Column(const string &label, const double &value, const int width)
          : label(label), valueptr(&value), width(width) {}
    };
   ...
   public:
    ...
    void print() {
      ...
      const double roundedLoadViol =
          myRound(clsCircuit.getLoadViolationUsingEffectiveCap(), 2);
      const double roundedSlewViol = myRound(clsCircuit.getSlewViolation(), 2);

      const double roundedTimingViol =
          myRound(clsCircuit.getTimingViolation(), 2);
      const double roundedWorstSlack = myRound(clsCircuit.getWorstSlack(), 2);

      cout << setw(N) << roundedWorstSlack << setw(N)
           << (clsCircuit.getLeakagePower() / 1e6) << setw(N)
           << roundedTimingViol << setw(N) << roundedSlewViol << setw(N)
           << roundedLoadViol << setw(N)
           << clsCircuit.getNumPathsWithNegativeSlack();
      ...
      // cout << "\t" << myRound(clsCircuit.getLoadViolationUsingEffectiveCap(),
      // 2) << " / " << myRound(clsCircuit.getLoadViolationUsingDownstreamCap(),
      // 2) << " / " <<
      // myRound(clsCircuit.getLoadViolationUsingEffectiveCapCellWise(), 2);
    ...
    void addExtraColumn(const string &label, const double &value,
                        const int width = 12) {
    ...
    ~DigestDescriptor() {
      ...
      const double x1 = myRound(
          100 * (clsCircuit.getNumPathsWithNegativeSlack() / double(x0)), 2);
      ...
      cout << "\tTiming Violation.: " << roundedTimingViol << " ["
           << clsCircuit.getTimingViolation()
           << "] #Paths: " << clsCircuit.getNumPathsWithNegativeSlack() << "/"
           << (x0) << " (" << x1 << "%)"
           << "\n";
      cout << "\tSlew Violation...: " << roundedSlewViol << " ["
           << clsCircuit.getSlewViolation() << "]"
           << "\n";
  ...
  struct TimingArcState {
    ...
    TimingArcState() { lambda.set(-1.0, -1.0); }  // end constructor
  };
  ...
  vector<pair<string, double> >
      footprintLEPairs;  // footprint and logical effort
 ...
 public:
  ...
  bool updateCellTypeLagrangeRelaxationLinearApproximation(
      Vcell *cell, const double alpha = 1.0);
  bool updateCellTypeLagrangeRelaxationSensitivitiesLinearApproximation(
      Vcell *cell, const double gamma, const double alpha = 1.0);
  bool updateCellTypeLagrangeRelaxationSensitivities(Vcell *cell,
                                                     const double gamma,
                                                     const double alpha = 1.0);
  bool updateCellTypeLagrangeRelaxationSensitivitiesDefault(
      Vcell *cell, const double gamma, const double alpha = 1.0);
  ...
  double computeLoadViolationUsingEffectiveCap(
      const TimingArc &arc, const TimingArcState &state) const;
  double computeLoadViolationUsingEffectiveCapCellWise(
      const TimingArc &arc, const TimingArcState &state) const;
  ...
  void sizingLagrangeRelaxationLinearApproximation(
      const bool resetLambdas = true);
  void sizingLagrangeRelaxationSensitivitiesLinearApproximation(
      const bool resetLambdas = true);
  void sizingLagrangeRelaxationLinearApproximationTestingChanges(
      const bool resetLambdas = true);
  void sizingLagrangeRelaxationSensitivities(const bool resetLambdas = true);
  void sizingLagrangeRelaxationSensitivitiesDefault(
      const bool resetLambdas = true, const int iterations = 150);
  void sizingLagrangeRelaxationSensitivitiesNew(const bool resetLambdas = true);
  void sizingLagrangeRelaxationSensitivitiesSomeChanges(
      const bool resetLambdas = true);
  ...
  void timingRecoveryPathCounter(const int limit,
                                 const bool highEffort = false);
  ...
  static void computeArcTiming(const LibParserTimingInfo &timingInfo,
                               ...
                               EdgeArray<double> &outSlew) {
    computeArcTiming(timingInfo, inputSlew, EdgeArray<double>(load, load),
                     outDelay, outSlew);
  }
  ...
  static bool nearlyZero(const double v, const double precision = EPSILON) {
  ...
  int getNumPathsWithNegativeSlack() const {
    return timingNumPathsWithNegativeSlack;
  }
  ...
  EdgeArray<double> getNetSlack(const int n) const {
    const TimingNetState &s = getTimingNetState(n);
    return s.requiredTime - s.arrivalTime;
  }
  EdgeArray<double> getNetNegativeSlack(const int n) const {
    const TimingNetState &s = getTimingNetState(n);
    return min(EdgeArray<double>(0, 0), s.requiredTime - s.arrivalTime);
  }
  EdgeArray<double> getNetPositiveSlack(const int n) const {
    const TimingNetState &s = getTimingNetState(n);
    return max(EdgeArray<double>(0, 0), s.requiredTime - s.arrivalTime);
  }
  ...
  string getTimingArcInputNodeName(const int k) {
    ...
    if (k < 0) {
      ...
      return arc.cell->instName + ":" +
             arc.cell->actualInstType->pins[arc.pin].name;
  ...
  double getLoadViolationUsingEffectiveCap() const {
    return timingViolationLoad;
  }
  double getLoadViolationUsingEffectiveCapCellWise() const {
    return timingViolationLoadCellWise;
  }
  ...
  void primeTimeConnect(const string &serverAddress,
