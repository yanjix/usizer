...
class TimerInterface {
 ...
 public:
  ...
  // 1. Write sizes
  // 2. Writes list pins for which timing data is requested (only if boolean
  // timing_request is set to true)
  //    (empty list means request is for all pins)
  // 3. Writes list of pins for which ceff data is requested (only if boolean
  // ceff_request is set to true)
  //    (empty list means request is for all pins)
  ...
  //         vector of string indicating pins/ports for which timing data is
  //         requested, empty vector means request is for all pins/ports boolean
  //         to indicate whether ceff data is requested vector of string
  //         indicating pins/ports for which ceff data is requested, empty
  //         vector means request is for all primary inputs and all cell
  //         instance outputs contest root directory (string) benchmark name
  //         (string) polling time (number of seconds that the function should
  //         wait before polling timer status to check whether timer is done)
  // Notes:  If both timing_request and ceff_request are set to false,
  // runTimingAnalysisBlocking will return TIMER_INTERFACEERROR
  //         as it is not expected that a user will call this function without
  //         expecting any returned timing or ceff information.
  // Return: timer status
  static Status runTimingAnalysisBlockingNegSlackOnly(
  ...
  static Status runTimingAnalysisBlockingNoReport(
      const std::vector<std::pair<std::string, std::string> > &sizes,
      const bool timing_request, const std::vector<std::string> &timing_pins,
      const bool ceff_request, const std::vector<std::string> &ceff_pins,
      const std::string &contest_root, const std::string &benchmark,
  ...
  static Status runTimingAnalysisBlockingTR(
      const std::vector<std::pair<std::string, std::string> > &sizes,
      const bool timing_request, const std::vector<std::string> &timing_pins,
      const bool ceff_request, const std::vector<std::string> &ceff_pins,
      const std::string &contest_root, const std::string &benchmark,
  ...
  static Status runTimingAnalysisBlockingPR(
      const std::vector<std::pair<std::string, std::string> > &sizes,
      const bool timing_request, const std::vector<std::string> &timing_pins,
      const bool ceff_request, const std::vector<std::string> &ceff_pins,
      const std::string &contest_root, const std::string &benchmark,
  ...
  // ceff_request is set to true)
  //    (empty list means request is for all pins)
  // 4. Starts timing analysis and returns (does not wait for timing analysis to
  // be completed) Inputs: vector of pairs where first value is instance name
  // (string) and second value is cell name (string)
  //         boolean to indicate whether timing data is requested
  //         vector of string indicating pins/ports for which timing data is
  //         requested, empty vector means request is for all pins/ports boolean
  //         to indicate whether ceff data is requested vector of string
  ...
  //         (string)
  // Notes:  If both timing_request and ceff_request are set to false,
  // startTimingAnalysisBlocking will return TIMER_INTERFACEERROR
  //         as it is not expected that a user will call this function without
  //         expecting any returned timing or ceff information.
  // Return: timer status
  ...
  static Status startTimingAnalysisNonBlockingNoReport(
      const std::vector<std::pair<std::string, std::string> > &sizes,
      const bool timing_request, const std::vector<std::string> &timing_pins,
      const bool ceff_request, const std::vector<std::string> &ceff_pins,
      const std::string &contest_root, const std::string &benchmark);
  ...
  // before checking timer status after calling startTimingAnalysisNonBlocking)
 ...
 private:
  ...
  // if timer is done) Input: directory name (string) Output: vector of file
  // names (strings), argument passed by reference Return: true if directory
  ...
  // startTimingAnalysis) Inputs: name of the file without directory name
  // (string)
  ...
  //         contest root directory (string)
  //         benchmark name (string)
  // Notes:  If both timing_request and ceff_request are set to false, then
  // writePinsForTimer will return false Return: true if pins were written
  // successfully to .timing_pins and .ceff_pins files, false otherwise
  static bool writePinsForTimer(const bool timing_request,
                                const std::vector<std::string> &timing_pins,
                                const bool ceff_request,
                                const std::vector<std::string> &ceff_pins,
                                const std::string &contest_root,
                                const std::string &benchmark);

  // Start timing analysis (does not wait for it to finish)
  // Input:  contest root directory (string)
  //         benchmark name (string)
  // Return: true if successfully wrote command to start timing analysis, false
  // otherwse
  static bool startTimingAnalysis(const std::string &contest_root,
                                  const std::string &benchmark);

  static bool startTimingAnalysisNegSlackOnly(const std::string &contest_root,
                                              const std::string &benchmark);

  static bool startTimingAnalysisNoReport(const std::string &contest_root,
                                          const std::string &benchmark);

  static bool startTimingAnalysisTR(const std::string &contest_root,
                                    const std::string &benchmark);

  static bool startTimingAnalysisPR(const std::string &contest_root,
                                    const std::string &benchmark);
  ...
  static Status runTimingAnalysisBlockingTR(const std::string &contest_root,
                                            const std::string &benchmark,
                                            const unsigned pollingTime);

  static Status runTimingAnalysisBlockingPR(const std::string &contest_root,
                                            const std::string &benchmark,
                                            const unsigned pollingTime);

  // Start timing analysis in non-blocking mode
  // 1. Starts timing analysis and returns (does not wait for timing analysis to
  // be completed) Input:  contest root directory (string)
  //         benchmark name (string)
  // Return: timer status
  static Status startTimingAnalysisNonBlocking(const std::string &contest_root,
                                               const std::string &benchmark);
