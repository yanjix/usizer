...
class TimingParser {
 ...
 public:
  ...
  // will be set to "".
  bool read_timing_line(string& name1, string& name2, double& riseSlack,
...
class CeffParser {
 ...
 public:
  ...
  // corresponds to a port, then name1 will be set to the port name, and name2
  // will be set to "".
  bool read_ceff_line(string& name1, string& name2, double& riseCeff,
                      double& fallCeff);
};
...
struct LibParserTimingInfo {
  ...
  // Note that ISPD-13 library will have only negative-unate combinational
  // cells. The clock arcs for sequentials will be non_unate (which can be
  // ignored because of the simplified sequential timing model for ISPD-13).
...
struct LibParserCellInfo {
  ...
  LibParserCellInfo()
      : leakagePower(0.0), area(0.0), isSequential(false), dontTouch(false) {}
};
...
class LibParser {
  ...
  void _begin_read_pin_info(string pinName, LibParserCellInfo& cell,
