...
class OrdCells {
 public:
  string footprint;
  double
      minimumFallSlew;  // minimum slew value for each footprint (type of cell)
  double minimumRiseSlew;

  double
      maximumFallSlew;  // maximum slew value for each footprint (type of cell)
...
class Wire {
 public:
  string wire_name;
  double cap;
  bool operator<(const Wire& w1) const { return (wire_name > w1.wire_name); }
  bool operator==(const Wire& w1) const { return (wire_name == w1.wire_name); }
  bool operator>(const Wire& w1) const { return (wire_name < w1.wire_name); }
  bool operator!=(const Wire& w1) const { return (wire_name != w1.wire_name); }
};
...
class AddrCell {
 public:
  ...
  bool operator<(const AddrCell& w1) const { return (instName < w1.instName); }
...
#define debug(tag, x) \
  if (app.hasOptionValuePair("tag", tag)) cout << x
 ...
 public:
  ...
  static string getOptionValue(const string& key, const int index = 0) {
    ...
    if (it != clsOptions.end()) {
      ...
      int counter = 0;
      for (set<string>::iterator its = optionParameters.begin();
           its != optionParameters.end(); ++its) {
        if (counter++ == index) return *its;
      }  // end for
    }    // end if
    return "";
  }  // end method
  ...
  static void parseCommandLineArguments(const int argc, char** argv,
    ...
    for (int r = offset; r < argc; r++) {
      ...
      if (arg[0] == '-') {
        ...
        OptionMap::iterator it = clsOptions.find(currentOption);
        if (it != clsOptions.end()) it->second.insert(arg);
      }  // end else
    }    // end for
  }      // end method
  ...
  static void printCommandLineArguments(ostream& out) {
    ...
    for (OptionMap::iterator it = clsOptions.begin(); it != clsOptions.end();
         ++it) {
      out << "\t" << it->first << "\t[";
